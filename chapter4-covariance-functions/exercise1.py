...
def main():
    ...
    plot_distribution(domain, *prior(domain), title="Prior (Ornstein-Uhlenbeck)")
    X = np.array([-4, -2, 0, 2, 4])
    f = np.array([0.5, -0.5, 0, -0.5, 0.5])
    plot_distribution(domain, *posterior(X, domain, f), training_points=(X, f), title=f"Posterior (Ornstein-Uhlenbeck)")
